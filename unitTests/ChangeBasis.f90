...
CALL ChangeBasis2D(nVar,1,nElems,2,5,NIn,NOut,Vdm,UIn2D,UOut2D(:,:,:,:,2),.FALSE.)
CALL ChangeBasis2D(nVar,1,nElems,2,5,NIn,NOut,Vdm,UIn2D,UOut2D(:,:,:,:,3),.TRUE.)
...
ELSE
  ...
  IF (binaryExists) THEN
    ...
    END DO
    IF (.NOT.equal) THEN
      WRITE(*,*) 'EROR - Calculated ChangeBasis deviate from reference.'
      STOP -1
    END IF
  ELSE
