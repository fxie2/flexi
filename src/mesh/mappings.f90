...
!==================================================================================================================================
!> Routine to prebuild and store mappings for specific polynomial degree
!==================================================================================================================================
  ...
  DO s = 1, 6
    DO q = 0,Nloc; DO p = 0,Nloc
      ...
      IF ((pq(1).NE.p).OR.(pq(2).NE.q)) THEN
        CALL abort(__STAMP__,'SideToVol does not fit to VolToSide')
      END IF
    END DO; END DO
  END DO ! s = 1, 6
END DO ! f = 0, 4
  ...
  DO s = 1, 6
    DO k=0,Nloc; DO j=0,Nloc; DO i=0,Nloc
      ...
      CASE(XI_MINUS,XI_PLUS)
        IF ((ijk(1).NE.j).OR.(ijk(2).NE.k)) THEN
          CALL abort(__STAMP__,'SideToVol2 does not fit to VolToSide')
        END IF
      CASE(ETA_MINUS,ETA_PLUS)
        IF ((ijk(1).NE.i).OR.(ijk(2).NE.k)) THEN
          CALL abort(__STAMP__,'SideToVol2 does not fit to VolToSide')
        END IF
      CASE(ZETA_MINUS,ZETA_PLUS)
        IF ((ijk(1).NE.i).OR.(ijk(2).NE.j)) THEN
          CALL abort(__STAMP__,'SideToVol2 does not fit to VolToSide')
        END IF
      END SELECT
