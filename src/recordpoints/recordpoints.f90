...
REAL,INTENT(OUT)              :: L_xi_RP(  0:PP_N,nRP)    !< Lagrange basis evaluated at recordpoints ($\xi$-direction)
REAL,INTENT(OUT)              :: L_eta_RP( 0:PP_N,nRP)    !< Lagrange basis evaluated at recordpoints ($\eta$-direction)
REAL,INTENT(OUT)              :: L_zeta_RP(0:PP_N,nRP)    !< Lagrange basis evaluated at recordpoints ($\zeta$-direction)
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER                       :: iRP
!==================================================================================================================================
...
IF(iter.EQ.0)THEN
  ...
  RP_Buffersize = MIN(CEILING((1.2*WriteData_dt)/(dt*RP_SamplingOffset)),RP_MaxBuffersize)
...
IF(myRPrank.EQ.0)THEN
  ...
  WRITE(UNIT_stdOut,'(a,I4,a)')' RP Buffer  : ',iSample,' samples.'
  GETTIME(startT)
END IF
