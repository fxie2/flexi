...
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
INTEGER,INTENT(IN) :: N_In,N_Out                 !< input/output polynomial degree
REAL,INTENT(IN)    :: xi_In(0:N_In)              !<  input nodal positions [-1,1]
REAL,INTENT(IN)    :: xi_Out(0:N_Out)            !< outout nodal positions [-1,1]
REAL,INTENT(IN)    :: wBary_In(0:N_In)           !< input interpolation weights
REAL,INTENT(OUT)   :: Vdm(0:N_Out,0:N_In)        !< nodal Vandermonde from N_In to N_out
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER            :: iXi
!==================================================================================================================================
...
REAL,INTENT(OUT)   :: L,Lder !< Legedre polynomial and deriv. evaluated at xi: L_N(xi), d/dxi L_N(xi)
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER :: iLegendre
...
REAL,INTENT(IN)    :: x                               !< coordinate value in the interval [-1,1]
REAL,INTENT(OUT)   :: L                               !< L_N(xi)
REAL,INTENT(OUT)   :: q                               !< q_N(xi)
REAL,INTENT(OUT)   :: qder                            !< d/dxi L_N(xi)
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER            :: iLegendre
REAL               :: L_Nm1,L_Nm2                     ! L_{N_in-2},L_{N_in-1}
...
!> of the Legendre Polynomial Lder_(N_in), which are the positions of Gauss points.
...
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
REAL,INTENT(IN) :: x,y         !< two scalar real numbers
...
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
REAL,INTENT(IN)              :: x                !< firstscalar to be compared
REAL,INTENT(IN)              :: y                !< second scalar to be compared
REAL,INTENT(IN)              :: tolerance        !< Tolerance to be checked against
LOGICAL                      :: EqualToTolerance !< TRUE if x and y are closer than tolerance 
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
REAL                        :: diff,maxInput
!===================================================================================================================================
