...
!> - Build Vandermonde matrizes
!> - Build derivative matrizes
!> Also contains routines to transformate between physical and reference space.
!==================================================================================================================================
...
!============================================================================================================================
!> Initialize interpolation. Call basis initialization and calculate Vandermonde matrizes between NUnder/N/Over.
!============================================================================================================================
...
IF (InterpolationInitIsDone) THEN
  CALL abort(__STAMP__,'InitInterpolation already called.')
END IF
...
IF(PP_N.NE.Ntmp)&
  CALL abort(__STAMP__,'N in ini-file is different from hard-compiled N in Flexi. Ini/Compiled:',Ntmp,REAL(PP_N))
#endif
...
!> Calculate positions of Gauss-points, integration weights and barycentric weights. Prepare basis evluated at -1 and +1.
...
LOGICAL,INTENT(IN),OPTIONAL        :: modal                      !> Switch if a modal Vandemonde should be build 
...
!==================================================================================================================================
!> Convert solution between physical <-> reference space, seperate input and output variables
!==================================================================================================================================
SUBROUTINE ApplyJacobian(U_in,U_out,toPhysical)
! MODULES
USE MOD_PreProc
...
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
REAL,INTENT(IN)    :: U_in( PP_nVar,0:PP_N,0:PP_N,0:PP_N,nElems) !> Input: Solution to be transformed
REAL,INTENT(OUT)   :: U_out(PP_nVar,0:PP_N,0:PP_N,0:PP_N,nElems) !> Output: transformed solution
...
IF(toPhysical)THEN
  DO iElem=1,nElems
    DO k=0,PP_N; DO j=0,PP_N; DO i=0,PP_N
      U_out(:,i,j,k,iElem)=U_in(:,i,j,k,iElem)*sJ(i,j,k,iElem)
...
!==================================================================================================================================
SUBROUTINE ApplyJacobian_local(U,toPhysical)
! MODULES
USE MOD_PreProc
USE MOD_Mesh_Vars,ONLY:sJ,nElems
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
REAL,INTENT(INOUT) :: U(PP_nVar,0:PP_N,0:PP_N,0:PP_N,nElems) !< Input/Output: Solution to be transformed 
LOGICAL,INTENT(IN) :: toPhysical                             !< Switch for physical<-->reference transformation
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER             :: i,j,k,iElem
