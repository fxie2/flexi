...
CALL WriteAdditionalData(FileName)
...
!> PLEASE DO NOT MISUSE DG FIELD DATA FOR THIS, AS IT WILL DRASTICALLY INCREASE FILE AND IO SIZE, USE THIS ROUTINE INSTEAD !!!!
!==================================================================================================================================
SUBROUTINE WriteAdditionalData(FileName)
! MODULES
USE MOD_PreProc
USE MOD_Globals
...
nVar = 0
nVar = nVar + 1 ! Timestep
! Count other additional variables here

! Allocate variable names and data array
ALLOCATE(VarNames(nVar))
ALLOCATE(ElemData(nVar,nElems))

! Fill varnames and data array
nVar = 1
VarNames(nVar)   = 'dt'
ElemData(nVar,:) = dtElem
! Fill other additional variables here
...
IF(MPIRoot)THEN
  CALL OpenDataFile(FileName,create=.FALSE.,single=.TRUE.,readOnly=.FALSE.)
  CALL WriteAttribute(File_ID,'VMSData',nVar,StrArray=VarNames)
  CALL CloseDataFile()
END IF
CALL GatheredWriteArray(FileName,create=.FALSE.,&
                        DataSetName='VMSData', rank=5,&
                        nValGlobal=(/1,PP_N+1,PP_N+1,PP_N+1,nGlobalElems/),&
                        nVal=      (/1,PP_N+1,PP_N+1,PP_N+1,nElems/),&
                        offset=    (/0,      0,     0,     0,     offsetElem/),&
                        collective=.TRUE.,RealArray=muSGS)

DEALLOCATE(ElemData,VarNames)
#endif

END SUBROUTINE WriteAdditionalData
...
USE MOD_PreProc
USE MOD_Globals
USE MOD_Output_Vars  ,ONLY: ProjectName,userblock_len,IniFilename
...
!==================================================================================================================================
! Create file
CALL OpenDataFile(TRIM(FileName),create=.TRUE.,single=.TRUE.,readOnly=.FALSE.,userblockSize=userblock_len)
...
CALL insert_userblock(TRIM(FileName)//C_NULL_CHAR,TRIM(IniFilename)//C_NULL_CHAR)
