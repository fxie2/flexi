...
CHARACTER(LEN=*),INTENT(IN) :: name !< (IN) incoming name, which is compared with the name of this option
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES 
LOGICAL            :: NAMEEQUALS
CHARACTER(LEN=255) :: name1
CHARACTER(LEN=255) :: name2
!==================================================================================================================================
CALL LowCase(this%name, name1)
CALL LowCase(name, name2)
NAMEEQUALS = (TRIM(name1).EQ.TRIM(name2))
...
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
REAL,INTENT(IN)         :: value         !< (IN) real value to print
INTEGER,INTENT(IN)      :: digits        !< (IN) number of digits (if < 1, then print as scientific)
INTEGER                 :: GETSTRLENREAL !< (OUT) length of real as string 
! LOCAL VARIABLES
...
CLASS IS (StringOption)
  ...
  END IF
CLASS IS (IntArrayOption)
  IF (maxValueLen - this%GETVALUELEN().GT.0) THEN
    WRITE(fmtValue,*) (maxValueLen - this%GETVALUELEN())
...
!==================================================================================================================================
!> parse value from string 'rest'. This subroutine is used to readin values from the parameter file.
!==================================================================================================================================
SUBROUTINE parse(this, rest) 
! MODULES
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
CLASS(OPTION)                 :: this
CHARACTER(LEN=255),INTENT(IN) :: rest
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES 
...
SELECT TYPE (this)
CLASS IS (IntOption)
  READ(rest, *) this%value
CLASS IS (LogicalOption)
  READ(rest, *) this%value
CLASS IS (RealOption)
  CALL parseReal(rest, this%value, this%digits)
CLASS IS (StringOption)
  READ(rest, "(A)") this%value
CLASS IS (IntArrayOption)
...
CLASS IS (RealArrayOption)
  ...
  DO 
    ...
    CALL parseReal(tmp, this%value(count), this%digits(count))
...
!===================================================================================================================================
SUBROUTINE parseReal(string_in, value, digits) 
IMPLICIT NONE
! INPUT / OUTPUT VARIABLES 
