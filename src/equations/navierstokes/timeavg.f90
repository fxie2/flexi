...
!> For the fluctuations we rely on the fact that U^'U^'= \bar(U)^2 - \bar(U^2)
!> The terms computed in this routine are therefore the TimeAvg: \bar(U) and the squared solution denoted by Fluc: \bar(U^2)
!==================================================================================================================================
...
IF((nVarAvg.EQ.0).AND.(nVarFluc.EQ.0)) CALL abort(__STAMP__, &
...
USE MOD_Equation_Vars,ONLY: Kappa,R
...
DO iElem=1,nElems
  ...
  IF(CalcAvg(11))THEN !'VelocitySound'
    DO k=0,PP_N; DO j=0,PP_N; DO i=0,PP_N
      tmpVars(iAvg(11),i,j,k)=SQRT(Kappa*prim(5,i,j,k)/prim(1,i,j,k))
    END DO; END DO; END DO
  END IF

  IF(CalcAvg(12))THEN !'Mach'
    DO k=0,PP_N; DO j=0,PP_N; DO i=0,PP_N
      tmpVars(iAvg(12),i,j,k)=SQRT(SUM(prim(2:4,i,j,k)**2)/(Kappa*prim(5,i,j,k)/prim(1,i,j,k)))
    END DO; END DO; END DO
  END IF

  IF(CalcAvg(13))THEN !'Temperature'
    DO k=0,PP_N; DO j=0,PP_N; DO i=0,PP_N
      tmpVars(iAvg(13),i,j,k)=prim(5,i,j,k)/(prim(1,i,j,k)*R)
    END DO; END DO; END DO
  END IF

  IF(CalcAvg(14))THEN !'TotalTemperature'
    DO k=0,PP_N; DO j=0,PP_N; DO i=0,PP_N
      Mach=SQRT(SUM(prim(2:4,i,j,k)**2)/(Kappa*prim(5,i,j,k)/prim(1,i,j,k)))
      tmpVars(iAvg(14),i,j,k)=prim(5,i,j,k)/(prim(1,i,j,k)*R)*(1.+0.5*(kappa-1)*Mach**2)
    END DO; END DO; END DO
  END IF
  ...
  IF(CalcAvg(15))THEN !'TotalPressure
    ...
    DO k=0,PP_N; DO j=0,PP_N; DO i=0,PP_N
      Mach=SQRT(SUM(prim(2:4,i,j,k)**2)/(Kappa*prim(5,i,j,k)/prim(1,i,j,k)))
      tmpVars(iAvg(15),:,:,:)=prim(5,:,:,:)/(1+0.5*(kappa-1)*Mach**2)**kappafac
    END DO; END DO; END DO
  END IF
...
IF(CalcFluc(17).OR.CalcFluc(18))THEN  !'Dissipation via vel gradients'
  ...
  END DO ! iElem
#else
  CALL abort(__STAMP__,'Cannot compute dissipation. Not compiled with parabolic flag.')
#endif /* PARABOLIC */
END IF
