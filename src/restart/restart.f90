...
IF((.NOT.InterpolationInitIsDone).OR.RestartInitIsDone)THEN
   CALL abort(__STAMP__,'InitRestart not ready to be called or already called.')
   RETURN
END IF
...
IF (nArgs .EQ. 2) THEN
  ...
  IF(.NOT.validHDF5) &
      CALL Abort(__STAMP__,'ERROR - Restart file not a valid state file.')
...
USE MOD_Interpolation,      ONLY: ApplyJacobian,GetVandermonde
...
IF(DoRestart)THEN
  ...
  ELSE
    ...
    IF(N_Restart.GT.PP_N)THEN
      ...
      END DO
      ! Transform back
      CALL ApplyJacobian(U,toPhysical=.TRUE.)
    ELSE
      DO iElem=1,nElems
        CALL ChangeBasis3D(PP_nVar,N_Restart,PP_N,Vdm_NRestart_N,U_local(:,:,:,:,iElem),U(:,:,:,:,iElem))
      END DO
